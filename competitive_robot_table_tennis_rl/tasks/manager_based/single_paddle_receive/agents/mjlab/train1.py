            ...
            while total_steps < cfg.timesteps:
                ...
                for step in range(rollout_len):
                    ...
                    if terminated or truncated:
                        ...
                        writer.add_scalar("performance/episode_return", current_episode_return, episode_idx)
                        writer.add_scalar("performance/episode_length", current_episode_length, episode_idx)
                        writer.add_scalar("performance/success", float(success), episode_idx)
                        writer.add_scalar("performance/rolling_return_mean_50", float(np.mean(recent_returns)), episode_idx)
                        writer.add_scalar("performance/rolling_length_mean_50", float(np.mean(recent_lengths)), episode_idx)
                        writer.add_scalar("performance/rolling_success_rate_50", float(np.mean(recent_successes)), episode_idx)
                        # writer.add_scalar("performance_by_step/episode_return", current_episode_return, total_steps)
                        # writer.add_scalar("performance_by_step/episode_length", current_episode_length, total_steps)
                        # writer.add_scalar("performance_by_step/success", float(success), total_steps)
                        # writer.add_scalar("performance_by_step/rolling_return_mean_50", float(np.mean(recent_returns)), total_steps)
                        # writer.add_scalar("performance_by_step/rolling_length_mean_50", float(np.mean(recent_lengths)), total_steps)
                        # writer.add_scalar("performance_by_step/rolling_success_rate_50", float(np.mean(recent_successes)), total_steps)

                        writer.add_scalar("exploration/base_policy_entropy_mean", float(np.mean(current_entropy_values)), episode_idx)
                        writer.add_scalar("exploration/action_std_mean", float(np.mean(current_action_std_values)), episode_idx)
                        writer.add_scalar("exploration/action_abs_mean", float(np.mean(current_action_abs_values)), episode_idx)
                        writer.add_scalar("exploration/action_saturation_frac", float(np.mean(current_action_saturation_values)), episode_idx)
                        writer.add_scalar("exploration/policy_mean_abs", float(np.mean(current_policy_mean_abs_values)), episode_idx)
                        # writer.add_scalar("exploration_by_step/base_policy_entropy_mean", float(np.mean(current_entropy_values)), total_steps)
                        # writer.add_scalar("exploration_by_step/action_std_mean", float(np.mean(current_action_std_values)), total_steps)
                        # writer.add_scalar("exploration_by_step/action_abs_mean", float(np.mean(current_action_abs_values)), total_steps)
                        # writer.add_scalar("exploration_by_step/action_saturation_frac", float(np.mean(current_action_saturation_values)), total_steps)
                        # writer.add_scalar("exploration_by_step/policy_mean_abs", float(np.mean(current_policy_mean_abs_values)), total_steps)

                        for key, value in current_reward_term_sums.items():
                            writer.add_scalar(f"environment/reward_terms/{key}", value, episode_idx)
                            writer.add_scalar(f"environment_by_step/reward_terms/{key}", value, total_steps)
                        writer.add_text("environment/terminal_reason/latest", terminal_reason, episode_idx)
                        for key, value in terminal_reason_counts.items():
                            writer.add_scalar(f"environment/terminal_reason_counts/{key}", value, episode_idx)
                ...
                writer.add_scalar("optimization/policy_loss", float(np.mean(policy_loss_values)), update_idx)
                writer.add_scalar("optimization/value_loss", float(np.mean(value_loss_values)), update_idx)
                writer.add_scalar("optimization/base_policy_entropy_mean", float(np.mean(entropy_loss_values)), update_idx)
                writer.add_scalar("optimization/total_loss", float(np.mean(total_loss_values)), update_idx)
                writer.add_scalar("optimization/approx_kl", float(np.mean(approx_kl_values)), update_idx)
                writer.add_scalar("optimization/clipfrac", float(np.mean(clipfrac_values)), update_idx)
                writer.add_scalar("optimization/grad_norm", float(np.mean(grad_norm_values)), update_idx)
                writer.add_scalar("optimization/param_norm", param_norm, update_idx)
                writer.add_scalar("optimization/explained_variance", explained_variance, update_idx)
                writer.add_scalar("optimization/raw_returns_mean", raw_returns_mean, update_idx)
                writer.add_scalar("optimization/raw_returns_std", raw_returns_std, update_idx)
                writer.add_scalar("optimization/advantage_mean_before_norm", float(adv_mean.detach().cpu()), update_idx)
                writer.add_scalar("optimization/advantage_std_before_norm", float(adv_std.detach().cpu()), update_idx)
                writer.add_scalar("optimization/total_steps", total_steps, update_idx)
                writer.add_scalar("optimization/update_epochs", cfg.update_epochs, update_idx)
                writer.add_scalar("optimization/minibatches_per_epoch", math.ceil(batch_size / minibatch_size), update_idx)
                writer.add_scalar("optimization/early_stop_on_kl", float(early_stop_triggered), update_idx)
                writer.add_scalar("optimization/learning_rate", optimizer.param_groups[0]["lr"], update_idx)

                writer.add_scalar("rollout/reward_sum", float(np.sum(reward_buf)), update_idx)
                writer.add_scalar("rollout/mean_reward", float(np.mean(reward_buf)), update_idx)
                writer.add_scalar("rollout/done_fraction", float(np.mean(done_buf)), update_idx)
                writer.add_scalar("rollout/value_mean", float(np.mean(value_buf)), update_idx)
                writer.add_scalar("rollout/value_std", float(np.std(value_buf)), update_idx)
                writer.add_scalar("rollout/advantage_mean_abs", float(np.mean(np.abs(advantages))), update_idx)
                for key, value in rollout_reward_term_sums.items():
                    writer.add_scalar(f"rollout/reward_terms/{key}", value, update_idx)
                writer.add_scalar("rollout/base_policy_entropy_mean", float(np.mean(rollout_entropy_values)), update_idx)
                writer.add_scalar("rollout/action_std_mean", float(np.mean(rollout_action_std_values)), update_idx)
                writer.add_scalar("rollout/action_abs_mean", float(np.mean(rollout_action_abs_values)), update_idx)
                writer.add_scalar("rollout/action_saturation_frac", float(np.mean(rollout_action_saturation_values)), update_idx)
                writer.add_scalar("rollout/policy_mean_abs", float(np.mean(rollout_policy_mean_abs_values)), update_idx)
                writer.add_scalar("system/steps_per_second", steps_per_second, update_idx)
                writer.add_scalar("system/train_elapsed_seconds", train_elapsed_s, update_idx)
                writer.add_scalar("system/episodes_completed", episode_idx, update_idx)
